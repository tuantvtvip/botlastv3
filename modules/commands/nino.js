...
module.exports.run = async ({ api, event, args, Threads }) => {
    ...
    if (!args[0]) { api.sendMessage("𝘂̉𝗮 𝗵𝗼̉𝗶 𝗷 𝗵𝗼̉𝗶 đ𝗶", threadID, messageID) } else {
        switch(args[0]) {
            case "on": {
                nino[threadID] = true;
                api.sendMessage("kích hoạt nino reply thành công bây giờ bạn có thể trò chuyện cùng bé rồi🐥!", threadID);
                break;
            }
            case "off": {
                nino[threadID] = false;
                api.sendMessage("nino phải off rồi hẹn gặp bé lần sau nhe :<", threadID);
                break;
            }
            default:
